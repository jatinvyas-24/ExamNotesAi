...
function History() {
  ...
  const openNotes = async (noteId) => {
    try {
      ...
      console.log("notes : ", response.data.content)
  ...
  return (
    <div className=" min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 px-6 py-8">
      ...
      <motion.header
        ...
        className="
       mb-10 rounded-2xl bg-black/80 backdrop-blur-xl border border-white/10 px-8 py-6 items-start flex justify-between md:items-center gap-4 flex-wrap
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <AnimatePresence>
          ...
          {(isSidebarOpen) && <motion.div
            ...
            className="fixed lg:static top-0 left-0 z-50 lg:rounded-3xl lg:z-auto w-72 lg:w-auto h-full lg:h-[75vh] lg:col-span-1 bg-black/90 lg:bg-black/80 backdrop-blur-xl border border-white/10 shadow-[0_20px_45px_rgba(0,0,0,0.6)] p-5 overflow-y-auto" >
            <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden  text-white mb-4 ">
              ⬅️ Back
            </button>

            <div className="mb-4 space-y-1">
              <button onClick={() => navigate("/notes")} className=" w-full text-start px-3  py-2 rounded-lg text-sm text-gray-200 bg-white/10 hover:bg-white/20 ">
                ➕ New Notes
              ...
              <ul className="space-y-3">
                ...
                {topics.map((t, i) => (
                  <li onClick={() => openNotes(t._id)} className={` cursor-pointer rounded-xl p-3 border transition-all 
                  ${
                    activeNoteId === t._id
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 text-xs ">
                      ...
                      {t.classLevel && <span className=" px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 ">
                        ClassLevel : {t.classLevel} </span>}

                      {t.examType && <span className=" px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 ">
                        ExamType : {t.examType} </span>}
                    </div>
